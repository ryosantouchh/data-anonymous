  ...
  return (
    <div className="fixed left-0 top-0 h-screen w-screen flex justify-center items-center bg-black bg-opacity-50">
      <div className="relative bg-white p-4 sm:p-6 rounded-xl w-[90%] sm:w-[685px]">
        <button
          className="font-semibold absolute right-4 top-2 sm:top-4"
        ...
        <div className="relative">
          <button
            className="w-full sm:w-auto mb-3 bg-white text-success rounded-lg flex items-center gap-1 font-semibold cursor-pointer px-4 h-[40px] border border-success justify-center"
            ...
            <ul className="absolute bg-white w-full sm:w-[200px] left-0 top-10 sm:top-10 shadow-gray-100 shadow-inner rounded-lg">
              {categories.map((category, index) => {
                return (
                  <li
                    key={category.id}
                    className="flex items-center justify-between py-2 px-3 hover:bg-green-100 font-medium"
                    onClick={() => {
                      setSelectedCommunity({
        ...
        <textarea
          className="mb-5 sm:mb-1 w-full h-[234px] p-3 border border-[#DADADA] resize-none focus:outline-none rounded-lg"
        ...
        <div className="flex flex-col sm:flex-row gap-2 justify-end">
          <button
            className={`bg-white rounded-lg w-full sm:w-[105px] h-[40px] border border-success`}
            onClick={() => setIsShowModal(false)}
          >
          ...
          <button
            className={`bg-success rounded-lg w-full sm:w-[105px] h-[40px] border-1 border-success`}
